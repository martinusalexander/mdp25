...
SharpIR sensor_L(sensor_L_pin,10,100,1080);
SharpIR sensor_C(sensor_C_pin,10,100,1080);
SharpIR sensor_R(sensor_R_pin,10,100,1080);
SharpIR sensor_SL(sensor_SL_pin,15,100,1080);
SharpIR sensor_SR(sensor_SR_pin,15,100,1080);
...
float str8_SpeedL, str8_SpeedR, rotate_SpeedL, rotate_SpeedR, L_offset;
double output;
...
//  Serial.println("MDP Group 10");
  ...
  str8_SpeedL = 250;    //250
  str8_SpeedR = 250;    //250
  rotate_SpeedL = 150;    //150
  rotate_SpeedR = 150;    //150
...
//  target_Tick = cmDis * 55.5;   //58.7
  ...
  if (cmDis<= 10) target_Tick = cmDis * 55.5;
  else if(cmDis<=20) target_Tick = cmDis * 58.5;    //done
  else if(cmDis<=30) target_Tick = cmDis * 58.5;
  else if(cmDis<=40) target_Tick = cmDis * 58.5;
  else if(cmDis<=50) target_Tick = cmDis * 58.5;    //ok
  else if(cmDis<=60) target_Tick = cmDis * 59.2;    //
  else if(cmDis<=70) target_Tick = cmDis * 58.5;
  else if(cmDis<=80) target_Tick = cmDis * 59.2;    //done
  else if(cmDis<=90) target_Tick = cmDis * 59.2;
  else if(cmDis<=100) target_Tick = cmDis * 59.2;   //done
  else if(cmDis<=110) target_Tick = cmDis * 59.2;
  else if(cmDis<=120) target_Tick = cmDis * 59.2;   //max
  else if(cmDis<=130) target_Tick = cmDis * 59.2; 
  else if(cmDis<=140) target_Tick = cmDis * 59.2;  
  else if(cmDis<=150) target_Tick = cmDis * 59.2;
...
//  target_Tick = angle * 8.76;     //or 8.79
  ...
  while (encoder_R_value < 300 )
  {
    output = tuneWithPID();
    md.setSpeeds(120 + output, -(120 - output));
  }

  while (encoder_R_value < 450 )
  {
    output = tuneWithPID();
    md.setSpeeds(140 + output, -(140 - output));
  }
  
  while (encoder_R_value < target_Tick - 450 ) 
  {
    output = tuneWithPID();
    md.setSpeeds(rotate_SpeedL + output, -(rotate_SpeedR - output));
  }

  while (encoder_R_value < target_Tick - 300 ) 
  {
    output = tuneWithPID();
    md.setSpeeds(140 + output, -(140 - output));
  }

  while (encoder_R_value < target_Tick - 150 ) 
  {
    output = tuneWithPID();
    md.setSpeeds(120 + output, -(120 - output));
  }
...
{
  RunningMedian sample = RunningMedian(17);
  for (int i = 0; i < 17; i ++)
  {
...
int gridsAwayL(int dis)
{
  ...
  else if (abs(dis) < 28) return 2;
  else if (abs(dis) < 42) return 3;
  else return -1;
}
...
int gridsAwayC(int dis)
{
  ...
  else if (abs(dis) < 41) return 3;    //44
  else return -1;
}
...
int gridsAwayR(int dis)
{
  ...
  else if (abs(dis) < 41) return 3;
  else return -1;
}

int gridsAwaySR(int dis)
{
  if (abs(dis) < 24) return 1;
  else if (abs(dis) < 34) return 2;     //or 36
  else if (abs(dis) < 48) return 3;     //54
//  else if (abs(dis) < 70) return 4;
  else return -1;
}

int gridsAwaySL(int dis)
{
  if (abs(dis) < 21) return 1;
  else if (abs(dis) < 29) return 2;   //31
  else if (abs(dis) < 47) return 3;
  else return -1;
}
/*-------------------------------------------------------------------
    RE-CENTER
